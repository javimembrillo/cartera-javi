...
let holdings={},prices={QDVE:42.52,TSLA:353.12,VWCE:165.62,SPCX:136};
let deposits={EUR:{tin:0},USD:{tin:0}};
let depositTxs=[],extraTxs=[],snapshots=[];
let dailyPrices={};
let eurusd=1.1685,history=[],investedStocksEUR=0,investedDepEUR=0;
...
function snapshotOn(dateStr,live){const qty={QDVE:0,TSLA:0,VWCE:0,SPCX:0};let invS=0;stockTxs().forEach(function(x){if(!x||!x.d||x.d>dateStr)return;qty[x.t]=(qty[x.t]||0)+x.q;invS+=-toEUR(x.a,x.c,x.d);});let vS=0;['QDVE','TSLA','VWCE','SPCX'].forEach(function(tk){const q=qty[tk];if(!q)return;const p=pxOn(tk,dateStr,live);if(p==null)return;vS+=CCY[tk]==='EUR'?q*p:q*p/fxOn(dateStr);});const vD=depValueEUROn(dateStr),invD=depInvestedEUROn(dateStr);return {v:vS+vD,inv:invS+invD,vStocks:vS,invStocks:invS,vDep:vD,invDep:invD,pl:(vS+vD)-(invS+invD)};}
function applyLedger(){holdings={QDVE:{qty:0,currency:'EUR',name:NAMES.QDVE},TSLA:{qty:0,currency:'USD',name:NAMES.TSLA},VWCE:{qty:0,currency:'EUR',name:NAMES.VWCE},SPCX:{qty:0,currency:'USD',name:NAMES.SPCX}};investedStocksEUR=0;history=[];const byDate={};stockTxs().forEach(function(x){(byDate[x.d]=byDate[x.d]||[]).push(x);});weekDates().forEach(function(d){(byDate[d]||[]).forEach(function(x){if(holdings[x.t])holdings[x.t].qty+=x.q;investedStocksEUR+=-toEUR(x.a,x.c,x.d);});let vStocks=0;Object.keys(holdings).forEach(function(tk){const q=holdings[tk].qty;if(!q)return;const p=pxOn(tk,d,false);if(p==null)return;vStocks+=holdings[tk].currency==='EUR'?q*p:q*p/fxOn(d);});const vDep=depValueEUROn(d);const invDep=depInvestedEUROn(d);history.push({t:new Date(d+'T15:00:00').getTime(),v:vStocks+vDep,inv:investedStocksEUR+invDep,vDep:vDep,invDep:invDep,vStocks:vStocks,invStocks:investedStocksEUR});});investedDepEUR=depInvestedEUROn(todayISO());}
...
function stocksNow(){return Object.keys(holdings).reduce(function(s,k){return s+valEUR(k);},0);}
...
function renderTotal(){const v=total(),invDep=depInvestedEUROn(todayISO()),inv=investedStocksEUR+invDep,plBolsa=stocksNow()-investedStocksEUR,plFondos=depNow()-invDep,pl=v-inv,pct=inv?pl/inv*100:0,pctB=investedStocksEUR?plBolsa/investedStocksEUR*100:0,pctF=invDep?plFondos/invDep*100:0;const tv=document.getElementById('totalValue');if(tv)tv.textContent=fmt(v)+' \u20ac';const plEl=document.getElementById('plLine');if(plEl)plEl.innerHTML='Invertido neto (acciones + dep\u00f3sitos): <b>'+fmt(inv)+' \u20ac</b><br>Acciones valor '+fmt(stocksNow())+' \u20ac \u00b7 invertido '+fmt(investedStocksEUR)+' \u20ac \u00b7 Dep\u00f3sitos valor '+fmt(depNow())+' \u20ac \u00b7 invertido '+fmt(invDep)+' \u20ac<br>P/L real bolsa: <span class="'+(plBolsa>=0?'pos':'neg')+'">'+(plBolsa>=0?'+':'')+fmt(plBolsa)+' \u20ac ('+fmt(pctB,1)+'%)</span> \u00b7 P/L real fondos: <span class="'+(plFondos>=0?'pos':'neg')+'">'+(plFondos>=0?'+':'')+fmt(plFondos)+' \u20ac ('+fmt(pctF,1)+'%)</span> \u00b7 P/L real total: <span class="'+(pl>=0?'pos':'neg')+'">'+(pl>=0?'+':'')+fmt(pl)+' \u20ac ('+fmt(pct,1)+'%)</span>';const sub=document.getElementById('totalSub');if(sub)sub.textContent='EUR/USD \u2248 '+fmt(eurusd,4);}
...
function renderAll(){try{renderHoldings();}catch(e){console.error(e);}try{renderDeps();}catch(e){console.error(e);}try{renderTotal();}catch(e){console.error(e);}try{renderDayMove();}catch(e){console.error(e);}try{updateProjections();}catch(e){console.error(e);}try{updateChart();}catch(e){console.error(e);}}
